...
void wf_make_block(wf_parameters *wf, double threshold, DKL_INT row_offset, DKL_INT block_size, double *buffer, double *binom, csr_sparse_matrix *Q, DKL_INT *current_size, DKL_INT *global_nnz, DKL_INT *old_size) {
  ...
  for (i = 0; i < block_size; i++) {
    ...
    for (j = 0; j < size; j++) {
      buffer[(i * size) + j] = (((fabs(buffer[(i * size) + j]) >= threshold) || (j == row_offset + i)) ? buffer[(i * size) + j] : 0);
    }
  ...
  *current_size += nnz;
  ...
  mkl_ddnscsr(job, &block_size, &size, buffer, &size, &(Q->data[*old_size]), &(Q->cols[*old_size]), Q->row_index, &info);
  assert(info == 0);
  dkl_dealloc(job);
}
...
csr_sparse_matrix *wf_matrix_csr(wf_parameters *wf, DKL_INT block_size, double threshold) {
  ...
  for (row_offset = 0; row_offset < (size - rem); row_offset += block_size) {
    wf_make_block(wf, threshold, row_offset, block_size, buffer, binom, Q, &current_size, global_nnz, &old_size);
  }
  wf_make_block(wf, threshold, row_offset, rem, buffer, binom, Q, &current_size, global_nnz, &old_size);
...
void wf_solve(wf_parameters *wf, wf_statistics *r, double zero_threshold, bool moran) {
  ...
  pardiso_control[MKL_PARDISO_REPORT_FLOP_FACTOR_PHASE] = MKL_PARDISO_REPORT_ENABLE;
  pardiso_control[MKL_PARDISO_REPORT_CGS_CG_DIAGNOSTIC] = MKL_PARDISO_REPORT_ENABLE;
  ...
  #endif
  pardiso_64(
    pardiso_internal, &pardiso_maximum_factors, &pardiso_matrix_number,
  ...
  for (int pp = stored_initial; pp <= max_value; pp++) {
    ...
    if (wf->observed_allele_count > 0) {
      ...
      pardiso_control[MKL_PARDISO_SOLVE_OPTION] = MKL_PARDISO_SOLVE_TRANSPOSED;
      pardiso_64(
        pardiso_internal, &pardiso_maximum_factors,
